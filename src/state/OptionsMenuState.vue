...
export const OptionsMenuState = reactive({
    ...
    getSafeMenuPosition(event:PointerEvent){
        ...
        if(typeof event.pointerType == 'undefined' || event.pointerType.trim() == '') menuSafePos = {x:clickTargetRect.x+(clickTargetRect.width/2),y:clickTargetRect.y+(clickTargetRect.height/2)};
